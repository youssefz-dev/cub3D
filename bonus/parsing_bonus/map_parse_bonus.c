...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/06 10:06:00 by noel-baz          #+#    #+#             */
/*   Updated: 2025/09/06 10:06:01 by noel-baz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3D_bonus.h"

char	*pad_map(char *line, int current_length, int max_width)
{
	int	spaces_needed;
...
int	extract_line(t_game *game, char *map_content, int *position,
	int line_index)
{
	int		j;
	char	*line;
	...
	if (!line)
		return (0);
	if (j < game->map->width)
		line = pad_map(line, j, game->map->width);
...
int	fill_map_grid(t_game *game, char *map_content, int map_height)
{
	...
	{
		if (!extract_line(game, map_content, &position,
				line_index))
		{
			while (line_index > 0)
			{
